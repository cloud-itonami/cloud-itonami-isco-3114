(ns elex.advisor
  "TestAdvisor — proposes an electronics test, inspection, or hazard-logging
  operation (record test/measurement data, log inspection findings, flag electronics
  ...
  project provenance or electronics-hazard risk; `elex.governor` is the independent
...
(def ^:private system-prompt
  "You are an electronics engineering technician advisor. Given an electronics
  ...
  (reify Advisor
    (-advise [_ _store request]
      (let [msgs [{:role :system :content system-prompt}
                  {:role :user :content (str "electronics operation request: " (pr-str request))}]
